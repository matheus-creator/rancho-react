...
const LoginBox = (props) => {
    ...
    const isPresent = (email, json) => {
        for (let i = 0; i < json.length; i++) {
            if (email === json[i].email) return i;
        }
    ...
    const validate = async (email, password, json) => {
        ...
        if (jsonIndex) {
            const response = await bcryptjs.compare(
                password,
                json[jsonIndex].password
            );
            return response;
        }
                ...
                <div className="forms">
                    ...
                    <div>
                        <Fade bottom collapse when={!state.valid}>
                            <div className="error">Credenciais inválidas</div>
                        </Fade>
                    </div>
                    ...
                    <button type="submit">{props.mainText}</button>
                </div>
            </form>
        </div>
    );
